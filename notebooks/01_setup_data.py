...
SELECT
    ...
    SUM(CASE WHEN ProductID LIKE '%OIL%' OR ProductID LIKE '%CRD%' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as total_oil_m3,
    SUM(CASE WHEN ProductID LIKE '%GAS%' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as total_gas_e3m3,
    SUM(CASE WHEN ProductID LIKE '%CND%' OR ProductID LIKE '%COND%' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as total_condensate_m3,
    SUM(CASE WHEN ProductID LIKE '%WTR%' OR ProductID LIKE '%WATER%' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as total_water_m3,
    SUM(TRY_CAST(Hours AS DOUBLE)) as total_hours,
...
FROM (
    SELECT
        ...
        COUNT(DISTINCT ProductionMonth) as active_months,
        SUM(TRY_CAST(GasProduction AS DOUBLE)) as total_gas_e3m3,
        SUM(TRY_CAST(OilProduction AS DOUBLE)) as total_oil_m3,
        SUM(TRY_CAST(CondensateProduction AS DOUBLE)) as total_condensate_m3,
        SUM(TRY_CAST(WaterProduction AS DOUBLE)) as total_water_m3,
        SUM(TRY_CAST(ResidueGasVolume AS DOUBLE)) as total_residue_gas_e3m3,
        SUM(TRY_CAST(Energy AS DOUBLE)) as total_energy_gj,
        SUM(TRY_CAST(EthaneMixVolume AS DOUBLE)) as total_ethane_mix_m3,
        SUM(TRY_CAST(EthaneSpecVolume AS DOUBLE)) as total_ethane_spec_m3,
        SUM(TRY_CAST(PropaneMixVolume AS DOUBLE)) as total_propane_mix_m3,
        SUM(TRY_CAST(PropaneSpecVolume AS DOUBLE)) as total_propane_spec_m3,
        SUM(TRY_CAST(ButaneMixVolume AS DOUBLE)) as total_butane_mix_m3,
        SUM(TRY_CAST(ButaneSpecVolume AS DOUBLE)) as total_butane_spec_m3,
        SUM(TRY_CAST(PentaneMixVolume AS DOUBLE)) as total_pentane_mix_m3,
        SUM(TRY_CAST(PentaneSpecVolume AS DOUBLE)) as total_pentane_spec_m3,
        SUM(TRY_CAST(LiteMixVolume AS DOUBLE)) as total_lite_mix_m3,
        SUM(TRY_CAST(Hours AS DOUBLE)) as total_hours,
        MIN(ProductionMonth) as first_production_month,
        MAX(ProductionMonth) as last_production_month,
...
SELECT
    ...
    SUM(CASE WHEN ActivityID = 'FLARE' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as flare_volume,
    SUM(CASE WHEN ActivityID = 'VENT' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as vent_volume,
    SUM(CASE WHEN ActivityID = 'FUEL' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as fuel_volume,
    SUM(CASE WHEN ActivityID IN ('FLARE','VENT','FUEL') THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as total_emissions_volume,
    SUM(CASE WHEN ActivityID = 'PROD' THEN TRY_CAST(Volume AS DOUBLE) ELSE 0 END) as production_volume
FROM {catalog}.{schema}.volumetrics
